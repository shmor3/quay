#!/usr/bin/env python3
"""
quay hot-reload client for Python

Connects to a running quay WebSocket server and executes callbacks
...
Usage:
    ...
    # Custom port
    python quay_client.py --port 4000

    # Custom host
    python quay_client.py --host 192.168.1.10 --port 3012
...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [quay] %(message)s",
    datefmt="%H:%M:%S",
)
log = logging.getLogger("quay")
...
class HotReloadClient:
    """Async WebSocket client that listens for quay messages.

    Parameters
    ----------
    host:
        Hostname or IP where quay is running.
    port:
        WebSocket port of the quay server (default ``3012``).
    on_reload:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="quay hot-reload client for Python")
    parser.add_argument(
        "--host",
        default=DEFAULT_HOST,
        help=f"quay server hostname (default: {DEFAULT_HOST})",
    )
    parser.add_argument(
        "--port",
        type=int,
        default=DEFAULT_PORT,
        help=f"quay WebSocket port (default: {DEFAULT_PORT})",
    )
    parser.add_argument(
        "--sync",
        action="store_true",
