// watchd hot-reload client for C# / .NET
//
// A standalone console application that connects to the watchd WebSocket
...
/// <summary>
/// A reusable hot-reload WebSocket client that connects to a watchd server
...
public class HotreloadClient : IDisposable
{
    ...
    public event EventHandler<HotreloadEventArgs>? OnReload;

    /// <summary>Raised when an "inject-css" message is received.</summary>
    public event EventHandler<HotreloadEventArgs>? OnInjectCss;

    /// <summary>Raised when any message is received (including unknown types).</summary>
    public event EventHandler<HotreloadEventArgs>? OnMessage;
    ...
    /// <summary>
    /// Creates a new HotreloadClient targeting the specified watchd server.
    /// </summary>
    /// <param name="host">Hostname or IP address (default: "localhost").</param>
    /// <param name="port">WebSocket port (default: 3012).</param>
    public HotreloadClient(string host = "localhost", int port = 3012)
    {
        _host = host;
        _port = port;
    }
    ...
    /// <summary>
    /// Connect to the watchd server and listen for messages indefinitely.
    ...
    private static void Log(string msg)
    {
        ...
        Console.WriteLine($"\u001b[1;31m[hotreload]\u001b[0m [{ts}] {msg}");
    }

    private static void Warn(string msg)
    {
        var ts = DateTime.Now.ToString("HH:mm:ss");
        Console.Error.WriteLine($"\u001b[1;33m[hotreload]\u001b[0m [{ts}] WARNING: {msg}");
    }
...
/// <summary>
/// Example console application demonstrating the HotreloadClient.
/// </summary>
public static class Program
{
    public static async Task Main(string[] args)
    {
        ...
        Console.WriteLine($"watchd hot-reload client for C# / .NET");
        ...
        using var client = new HotreloadClient(host, port);
